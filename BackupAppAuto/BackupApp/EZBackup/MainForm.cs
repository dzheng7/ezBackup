    ...
    public partial class MainForm : Form
    {
        ...
        {//Things that need to happen before the window appears.
            ...
            else
            {
                ...
                writer.Close();
                MessageBox.Show("Fill in the info first");
            }
        }
        ...
        public void backupFiles()
        {
            ...
            finalFolder2 = destinationPath;
            textFilePath = originPath;
            if (destinationPath.Trim() != "" && originPath.Trim() != "")
            {//If both text boxes are filled
                if (Directory.Exists(destinationPath) //The backup to folder indicated exists
                    && (File.Exists(originPath) || Directory.Exists(originPath) || indivSel))
        ...
        //private void nullRadio_CheckedChanged(object sender, EventArgs e)
        //{
        //    if (nullRadio.Checked == true)
        //    {
        //        oriButton.Enabled = false;
        //        textBox2.Enabled = false;
        //        picturePanel.BackgroundImage = null;
        ...
        {//(Backup from)/(File list)
            ...
            {//If this is the multiple folder option
                ...
                else
                {
                    ...
                    if(numFolders != 1)
                        originPath = numFolders.ToString() + " folders to backup";
                    else
                        originPath = "1 folder to backup";
                    button1.Visible = true;
                    button2.Visible = true;
                    folderList = folderPaths;
                    string x = "";
            ...
            {//If this is the single folder option
                ...
                {
                    originPath = folderBrowserDialog1.SelectedPath;
                }
            }
        ...
        {//A bunch of code for the actual backing up
            ...
            textFilePath = originPath;
            if (textBox1.Text.Trim() != "" && originPath.Trim() != "")
            {//If both text boxes are filled
                ...
                {//Error messages for if at least one of the text boxes isn't valid
                    if (!Directory.Exists(textBox1.Text) && !Directory.Exists(originPath)
                        && !File.Exists(originPath))
                    {//If neither are valid
                        //MessageBox.Show("Please insert valid paths");
                    }
        ...
        private void textBox2_TextChanged(object sender, EventArgs e)
        {
            ...
            if (originPath.Trim() != "")
            {
                if (!Directory.Exists(originPath))
                {
                    if ((textRadio.Checked && !File.Exists(originPath)) || !indivRadio.Checked)
                    {
                        picturePanel.BackgroundImage = Properties.Resources.wrong_cross_clip_art;
                    }
                }
                if (singleRadio.Checked && Directory.Exists(originPath))
                {
                    picturePanel.BackgroundImage = Properties.Resources.folder_generic_small;
                }
                if (multiRadio.Checked && textRadio.Checked && File.Exists(originPath))
                {
